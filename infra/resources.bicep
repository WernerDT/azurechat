...
var openai_name = toLower('${name}ai-tfgpt')
var form_recognizer_name = toLower('${name}-form-tfgpt')
var speech_service_name = toLower('${name}-speech-tfgpt')
var cosmos_name = toLower('${name}-cosmos-tfgpt')
var search_name = toLower('${name}search-tfgpt')
var webapp_name = toLower('${name}-webapp-tfgpt')
var appservice_name = toLower('${name}-app-tfgpt')
// keyvault name must be less than 24 chars - token is 13
var kv_prefix = take(name, 7)
var keyVaultName = toLower('${kv_prefix}-kv-tfgpt')
var la_workspace_name = toLower('${name}-la-tfgpt')
